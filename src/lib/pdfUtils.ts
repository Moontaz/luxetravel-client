...
export const generateTicketPDF = async (ticket: Ticket): Promise<Blob> => {
  ...
  try {
    ...
    const departureTime = ticket.bus_details?.departure_time
      ? (() => {
          try {
            return new Date(ticket.bus_details.departure_time).toLocaleString(
              "id-ID",
              {
                weekday: "long",
                year: "numeric",
            ...
            return ticket.bus_details.departure_time;
          }
        })()
      : ticket.bus?.departureTime
      ? (() => {
          try {
            return new Date(ticket.bus.departureTime).toLocaleString("id-ID", {
              weekday: "long",
              year: "numeric",
              month: "long",
              day: "numeric",
            ...
            return ticket.bus.departureTime.toString();
          }
        })()
      : "Time not available";
